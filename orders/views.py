...
from .serializers import (
    OrderItemSerializer, CartSummarySerializer, CartItemSerializer,
    OrderListSerializer, OrderDetailSerializer, OrderDeliverySerializer, DriverOrderSerializer
)
...
class DriverOrdersListView(generics.ListAPIView):
    ...
    def get_queryset(self):
        ...
        driver_profile = getattr(self.request.user, 'deliverer_profile', None) # Changed to deliverer_profile
...
class DriverOrderAcceptView(APIView):
    ...
    def post(self, request, pk):
        ...
        order = get_object_or_404(Order, pk=pk, driver=getattr(request.user, 'deliverer_profile', None)) # Changed to deliverer_profile
...
class DriverOrderStatusUpdateView(APIView):
    ...
    def post(self, request, pk):
        ...
        if status_update not in allowed:
            return Response({"detail": f"Invalid status. Must be one of {allowed}."}, status=status.HTTP_400_BAD_REQUEST)
        ...
        return Response({"detail": f"Order cannot be updated to {status_update} at its current stage ({order.status})."}, status=status.HTTP_400_BAD_REQUEST)
...
class DriverOrderArrivalView(APIView):
    ...
    def post(self, request, pk):
        ...
        try:
            ...
            return Response({"detail": "wait_seconds must be a non-negative integer."}, status=status.HTTP_400_BAD_REQUEST)
        ...
        if order.status in ['On The Way']:
            ...
            order.save()
            order_delivery.save()
            return Response({"detail": f"Order {pk} arrival time and wait seconds recorded."}, status=status.HTTP_200_OK)

        return Response({"detail": "Order cannot be marked as arrived at this stage."}, status=status.HTTP_400_BAD_REQUEST)
...
class DriverOrderViewSet(viewsets.ViewSet):
    ...
    def list(self, request):
        ...
        if not request.user.is_authenticated:
            return Response([], status=status.HTTP_200_OK)
        driver_profile = getattr(request.user, 'deliverer_profile', None) # Changed to deliverer_profile
        if driver_profile is None:
            return Response([], status=status.HTTP_200_OK)
        qs = Order.objects.filter(driver=driver_profile).exclude(status__in=['Delivered', 'Canceled', 'Returned']).order_by('-created_at')
