...
class DelivererAdmin(admin.ModelAdmin):
    ...
    def save_model(self, request, obj, form, change):
        ...
        if not obj.pk:
            ...
            OnboardToken.objects.create(
                user=obj.user, token_hash=token_hash, expires_at=expires
            )

            # Construct onboarding link
            onboarding_link = request.build_absolute_uri(
                reverse("deliverer_onboarding_verify")
                + f"?token={onboarding_token_str}&deliverer_id={obj.id}"
            )
            ...
            try:
                ...
                self.message_user(
                    request,
                    f"Deliverer {obj.user.email} created. Onboarding email sent.",
                )
            except Exception as e:
                self.message_user(
                    request, f"Error sending onboarding email: {e}", level="error"
                )
