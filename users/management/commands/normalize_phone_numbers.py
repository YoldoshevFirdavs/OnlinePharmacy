...
from users.models import (  # NOTE: Import models that might have phone numbers
    CustomUser,
    Deliverer,
)
...
class Command(BaseCommand):
    help = (
        "Normalizes phone numbers in CustomUser and Deliverer models to E.164 format."
    )
    ...
    def handle(self, *args, **options):
        ...
        )  # NOTE: Get default region from settings
            ...
            if user.phone_number:
                try:
                    parsed_number = phonenumbers.parse(
                        user.phone_number, default_region
                    )
                    if phonenumbers.is_valid_number(parsed_number):
                        ...
                        if user.phone_number != e164_number:
                            ...
                            self.stdout.write(
                                self.style.SUCCESS(
                                    f"Normalized user {user.id} phone: {e164_number}"
                                )
                            )
                    else:
                        self.stdout.write(
                            self.style.WARNING(
                                f"Invalid phone number for user {user.id}: {user.phone_number}"
                            )
                        )
                except phonenumbers.phonenumberutil.NumberParseException:
                    self.stdout.write(
                        self.style.ERROR(
                            f"Could not parse phone number for user {user.id}: {user.phone_number}"
                        )
                    )
            else:
                self.stdout.write(
                    self.style.NOTICE(f"User {user.id} has no phone number.")
                )
        ...
        if users_to_update:
            ...
            self.stdout.write(
                self.style.SUCCESS(
                    f"Successfully updated {len(users_to_update)} CustomUser phone numbers."
                )
            )
        else:
            self.stdout.write(
                self.style.INFO("No CustomUser phone numbers needed normalization.")
            )
        ...
        for deliverer in Deliverer.objects.all():
            if (
            ...
            ):  # Assuming Deliverer has a direct phone_number field
                try:
                    parsed_number = phonenumbers.parse(
                        deliverer.phone_number, default_region
                    )
                    if phonenumbers.is_valid_number(parsed_number):
                        e164_number = phonenumbers.format_number(
                            parsed_number, phonenumbers.PhoneNumberFormat.E164
                        ...
                        if deliverer.phone_number != e164_number:
                            ...
                            self.stdout.write(
                                self.style.SUCCESS(
                                    f"Normalized deliverer {deliverer.id} phone: {e164_number}"
                                )
                            )
                    else:
                        self.stdout.write(
                ...
                self.stdout.write(
                    self.style.NOTICE(f"Deliverer {deliverer.id} has no phone number.")
                )
        ...
        if deliverers_to_update:
            ...
            self.stdout.write(
                self.style.SUCCESS(
                    f"Successfully updated {len(deliverers_to_update)} Deliverer phone numbers."
                )
            )
        else:
            self.stdout.write(
                self.style.INFO("No Deliverer phone numbers needed normalization.")
            )
