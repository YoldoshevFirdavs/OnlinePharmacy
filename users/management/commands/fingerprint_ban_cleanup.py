...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        try:
            ...
            if not any([options.get('stats'), options.get('cleanup'), 
                       options.get('clear_rate_limits'), options.get('clear_ip_blocks'),
    ...
    def _show_stats(self):
        ...
        try:
            ...
            self.stdout.write(
                self.style.ERROR(f'✗ Statistika xatoligi: {str(e)}')
            )
    ...
    def _cleanup_expired_bans(self, dry_run=False):
        ...
        try:
            ...
            if dry_run:
                ...
                for fp, ban_info in expired_fps:
                    ...
                    if success:
                        reason = ban_info.get('ban_reason', 'N/A')
                        self.stdout.write(
                            self.style.SUCCESS(f'✓ {fp[:8]}... unbanned: {reason}')
                        )
            ...
            self.stdout.write(
                self.style.ERROR(f'✗ Cleanup xatoligi: {str(e)}')
            )
    ...
    def _clear_rate_limits(self, dry_run=False):
        ...
        try:
            ...
            self.stdout.write(
                self.style.ERROR(f'✗ Rate limit tozalash xatoligi: {str(e)}')
            )
    ...
    def _clear_ip_blocks(self, dry_run=False):
        ...
        try:
            ...
            self.stdout.write(
                self.style.ERROR(f'✗ IP blok tozalash xatoligi: {str(e)}')
            )
    ...
    def _show_fingerprint_info(self, fingerprint):
        ...
        try:
            ...
            self.stdout.write(
                self.style.ERROR(f'✗ Fingerprint ma\'lumot xatoligi: {str(e)}')
            )
    ...
    def _get_redis_client(self):
        ...
        try:
            ...
            if isinstance(cache, RedisCache):
                ...
                self.stdout.write(
                    self.style.WARNING('⚠️  Redis cache ishlatilmayapti')
                )
                return None
        except Exception as e:
