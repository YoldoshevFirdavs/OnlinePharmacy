...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        try:
            ...
            if total_unbanned == 0:
                ...
                self.stdout.write(
                    self.style.SUCCESS(f'✓ Jami {total_unbanned} ta ban ochildi')
                )
                
        except Exception as e:
            self.stdout.write(
                self.style.ERROR(f'✗ Xatolik: {str(e)}')
            )
    ...
    def _handle_user_bans(self, dry_run=False):
        ...
        try:
            ...
            if dry_run:
                self.stdout.write(
                    self.style.WARNING(f'[DRY RUN] {count} ta user ban ochiladi:')
                )
                for user in expired_bans:
                    self.stdout.write(
                        f"  - User {user.id}: {user.email} (ban sababi: '{user.banned_for}')"
                    )
                return count
            ...
            for user in expired_bans:
                ...
                self.stdout.write(
                    self.style.SUCCESS(f'✓ User {user.id} ({user.email}) unbanned')
                )
            ...
            self.stdout.write(
                self.style.ERROR(f'✗ User ban tekshirishda xatolik: {str(e)}')
            )
    ...
    def _handle_fingerprint_bans(self, dry_run=False):
        ...
        try:
            ...
            if dry_run:
                self.stdout.write(
                    self.style.WARNING(f'[DRY RUN] {count} ta fingerprint ban ochiladi:')
                )
                for fp, ban_info in expired_fps:
                    reason = ban_info.get('ban_reason', 'Noma\'lum')
                    self.stdout.write(
                        f"  - Fingerprint {fp[:8]}...: {reason}"
                    )
                return count
            ...
            for fp, ban_info in expired_fps:
                ...
                if success:
                    reason = ban_info.get('ban_reason', 'Noma\'lum')
                    self.stdout.write(
                        self.style.SUCCESS(f'✓ Fingerprint {fp[:8]}... unbanned (reason: {reason})')
                    )
            ...
            self.stdout.write(
                self.style.ERROR(f'✗ Fingerprint ban tekshirishda xatolik: {str(e)}')
            )
