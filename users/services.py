...
class BanService:
    ...
    def unban_by_fp(fp, actor=None):
        ...
        try:
            ...
            logger.warning(
                f"[UNBAN_FP] Fingerprint {fp[:8]}... unbanned by {actor_name} "
    ...
    def record_blocked_event(user, path_attempted, reason, banned_for=None):
        ...
        try:
            log_msg = (
                f"[BLOCKED] User {user.id} ({user.email}) tried to access {path_attempted} - "
                f"Reason: {reason}"
            )
            if banned_for:
                log_msg += f" - Banned for: {banned_for}"
    ...
    @staticmethod
    def increment_failed_attempts(user, field='failed_telegram_attempts', limit=None, ban_page='telegram_check', ban_duration_minutes=60):
        """
    ...
    def approve_unban_request(request_id, approver_id, is_superuser=False):
        ...
        if request_info['requester_id'] == approver_id:
            return {'success': False, 'message': "O'zingizni unban qila olmaysiz. Iltimos boshqa admindan tasdiq oling.", 'action': 'self_approval_forbidden'}
        
        # For permanent bans, only superuser can approve
        if request_info.get('is_permanent') and not is_superuser:
            return {'success': False, 'message': 'Permanent banlarni faqat superadmin tasdiqlashi mumkin', 'action': 'insufficient_permissions'}
    ...
    def reject_unban_request(request_id, rejection_reason='', approver_id=None):
        ...
        logger.warning(
            f"[UNBAN_REJECTED] Admin {approver_id} rejected unban request {request_id} "
    ...
    def approve_revert_request(request_id, approver_id, is_superuser=False):
        ...
        logger.warning(
            f"[REVERT_APPROVED] Admin {approver_id} approved revert request {request_id}"
        )
        
        return {'success': revert_result, 'message': 'Revert approved' if revert_result else 'Revert failed', 'action': 'approved'}
