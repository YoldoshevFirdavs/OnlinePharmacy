...
class DeviceFingerprintMiddlewareTest(DeviceFingerprintTestCase):
    ...
    @override_settings(
        FINGERPRINT_RATE_THRESHOLD=2,  # Low threshold for testing
    ...
    def test_rate_limiting(self):
        ...
        response1 = self.client.get('/', **headers)
        self.assertNotEqual(response1.status_code, 302)  # Not redirected
        
        response2 = self.client.get('/', **headers)
        self.assertNotEqual(response2.status_code, 302)  # Not redirected
        
        # Third request should trigger rate limit
        response3 = self.client.get('/', **headers)
        
        # Should be redirected to not-allowed page or banned
        # The exact behavior depends on middleware processing order
        # Let's check if the fingerprint got banned
        self.assertTrue(BanService.is_fp_banned(self.test_fingerprint))
    
    def test_banned_fingerprint_blocked(self):
        """Test that banned fingerprints are blocked"""
        ...
        # Try to access with banned fingerprint
        headers = {
            'HTTP_AUTHORIZATION_FINGERPRINT': self.test_fingerprint,
            'REMOTE_ADDR': self.test_ip
        }
    ...
    def test_ip_blocking_after_rate_limit(self):
        ...
        # Try to access from blocked IP (even with different fingerprint)
        different_fp = 'different_fingerprint_1234567890abcdef'
        headers = {
            'HTTP_AUTHORIZATION_FINGERPRINT': different_fp,
            'REMOTE_ADDR': self.test_ip
        }
...
class AdminAPITest(DeviceFingerprintTestCase):
    ...
    def test_unban_fingerprint_api_admin_required(self):
        ...
        response = self.client.post(
            '/dashboard/api/admin/unban-fingerprint/',
    ...
    def test_clear_ip_block_api(self):
        ...
        with patch('dashboard.admin_api_views.get_client_ip', return_value=self.test_ip):
            response = self.client.post('/dashboard/api/admin/clear-ip-block/')
    ...
    def test_fingerprint_ban_status_api(self):
        ...
        response = self.client.get('/dashboard/api/admin/fingerprint-ban-status/')
        self.assertEqual(response.status_code, 200)
        
        data = json.loads(response.content)
        self.assertTrue(data['banned'])
        self.assertEqual(data['ban_info']['reason'], 'API test ban')
...
class NotAllowedViewTest(DeviceFingerprintTestCase):
    ...
    def test_not_allowed_shows_user_ban_info(self):
        ...
        BanService.ban_user(
            self.test_user,
            duration_minutes=60,
            reason='User ban test',
            banned_for='user_testing'
        )
