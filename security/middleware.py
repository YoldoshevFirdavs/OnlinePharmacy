...
class SafeErrorMiddleware(MiddlewareMixin):
    ...
    def __call__(self, request):
        ...
        if response.status_code >= 400 and not settings.DEBUG:
            ...
            return JsonResponse({"error": "Login failed, please try again."}, status=response.status_code)
        return response
    ...
    def process_exception(self, request, exception):
        ...
        if isinstance(exception, APIException):
            ...
            if not settings.DEBUG:
                return JsonResponse({"error": "Login failed, please try again."}, status=exception.status_code)
