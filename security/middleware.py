...
class BanMiddleware:
    ...
    def __call__(self, request):
        ...
        # Check if banned (comprehensive check)
        # Pass all identifiers so ban system can do smart matching
        ban = self._get_active_ban(ip=ip, fingerprint=fp, user=user if user and user.is_authenticated else None)
        ...
        if self._check_and_update_rate_limit(ip, fp):
            # Create temporary ban with all identifiers
            ...
            logger.warning(f"[BAN] Temporary ban created for IP {ip}, FP {fp}: {ban.reason}")
    ...
    def _get_active_ban(self, ip=None, fingerprint=None, user=None):
        """Get active ban for IP, fingerprint, yoki user"""
        return BanRecord.get_active_ban(ip=ip, fingerprint=fingerprint, user=user)
    ...
    def _log_ban_attempt(self, ip, ban):
        ...
        logger.warning(
            f"[BAN] IP {ip} blocked - Type: {ban.get_ban_type_display()}, Reason: {ban.reason}, Created: {ban.created_by}"
        )
