...
class BanRecordAdmin(admin.ModelAdmin):
    list_display = ['identifier', 'ban_type_badge', 'created_at', 'expires_at', 'attempts', 'created_by', 'is_active_badge']
    ...
    fieldsets = (
        (_('Identifiers'), {
            'fields': ('ip', 'fingerprint', 'user')
        }),
        (_('Ban Info'), {
            'fields': ('reason', 'ban_type', 'created_by', 'source')
        }),
        (_('Timing'), {
            'fields': ('created_at', 'expires_at')
        }),
        (_('Details'), {
            'fields': ('attempts', 'meta_display', 'is_active')
        }),
    )
    ...
    def identifier(self, obj):
        """Show IP, fingerprint, or user"""
        if obj.ip:
            return f"IP: {obj.ip}"
        elif obj.fingerprint:
            return f"FP: {obj.fingerprint[:20]}..."
        elif obj.user:
