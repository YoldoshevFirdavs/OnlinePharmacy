...
from security.ip_score import get_ip_score, reset_ip_score, incr_ip_score, decr_ip_score
import logging
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if reset:
            ...
            self.stdout.write(self.style.SUCCESS(f'Successfully incremented score for IP: {ip_address}. New score: {new_score}'))
        elif decr_delta > 0:
            new_score = decr_ip_score(ip_address, delta=decr_delta)
            self.stdout.write(self.style.SUCCESS(f'Successfully decremented score for IP: {ip_address}. New score: {new_score}'))
        else:
