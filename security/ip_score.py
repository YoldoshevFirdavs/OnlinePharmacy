...
def incr_ip_score(ip: str, delta: int = 10, ttl: int = 86400):
    ...
    if new_score >= BLOCK_IP_SCORE:
        logger.critical('IP %s score reached BLOCKING threshold (%s/%s).', ip, new_score, BLOCK_IP_SCORE)
    elif new_score >= HIGH_RISK_IP_SCORE:
        logger.warning('IP %s score reached HIGH RISK threshold (%s/%s).', ip, new_score, HIGH_RISK_IP_SCORE)
