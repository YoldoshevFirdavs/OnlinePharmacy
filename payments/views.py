...
def stripe_webhook(request):
    ...
    if event['type'] == 'transfer.succeeded':
        ...
        try:
            ...
            payout.save()
            logger.info(f"Stripe Transfer Succeeded: Payout {payout.id} updated to completed.")
        except Payout.DoesNotExist:
            logger.warning(f"Stripe Transfer Succeeded: Payout with transfer ID {transfer['id']} not found.")
        ...
        try:
            ...
            payout.save()
            logger.warning(f"Stripe Transfer Failed: Payout {payout.id} updated to failed.")
        except Payout.DoesNotExist:
            logger.warning(f"Stripe Transfer Failed: Payout with transfer ID {transfer['id']} not found.")
    else:
...
def create_payout(request):
    ...
    if net_amount < 0:
        return Response({'detail': 'Net amount cannot be negative.'}, status=status.HTTP_400_BAD_REQUEST)

    if not driver_profile.stripe_account_id:
        return Response({'detail': 'Driver does not have a connected Stripe account for payouts.'}, status=status.HTTP_400_BAD_REQUEST)
    ...
    try:
        ...
        transfer = stripe.Transfer.create(
            ...
            metadata={'driver_id': str(driver_profile.id), 'user_id': str(driver_profile.user.id)}
        )
        ...
        return Response({'detail': f'Stripe transfer failed: {e}'}, status=status.HTTP_500_INTERNAL_SERVER_ERROR)
...
class DriverPayoutListView(APIView):
    ...
    def get(self, request):
        driver = getattr(request.user, 'delivery_driver_profile', None)
        if not driver:
            return Response([], status=status.HTTP_200_OK)
        payouts = Payout.objects.filter(driver=driver).order_by('-created_at')
