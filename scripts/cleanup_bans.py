...
class BanCleanupManager:
    ...
    def emergency_cleanup(self):
        ...
        if confirmation == "YES":
            ...
            # Clear rate limits
            if not self.run_django_command(
                "fingerprint_ban_cleanup --clear-rate-limits", "Emergency: Clear rate limits"
            ):
                success = False
    ...
    def check_fingerprint(self, fingerprint):
        ...
        return self.run_django_command(
            f"fingerprint_ban_cleanup --fingerprint {fingerprint}", f"Fingerprint check: {fingerprint[:8]}..."
        )
...
Examples:
  ...
  %(prog)s setup-scheduler      # Show scheduler setup instructions
    ...
    parser.add_argument("fingerprint", nargs="?", help="Fingerprint to check (for check action)")
