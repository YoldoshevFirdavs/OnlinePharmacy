...
from pharmacy.models.comments import ProductComment, CommentLike, CommentAnalysis
...
class CommentLikeModelTestCase(TestCase):
    ...
    def setUpTestData(cls):
        ...
        cls.product = Medicine.objects.create(
            name="Test", slug="test", category=cls.category, price=100.0
        )
    ...
    def test_add_emoji_reaction(self):
        ...
        reaction = CommentLike.objects.create(
...
class ProductCommentAPITestCase(APITestCase):
    ...
    def test_unlike_comment(self):
        ...
        self.client.force_authenticate(user=self.user2)
        response = self.client.post(
            f'/api/v1/comments/{comment.id}/unlike/',
            {'emoji': 'heart'}
        )
