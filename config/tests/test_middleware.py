...
class DeviceFingerprintMiddlewareTest(TestCase):
    ...
    def test_banned_fingerprint_redirect(self):
        ...
        # Ban the fingerprint
        BanService.ban_by_fp(self.test_fingerprint, duration_minutes=60, reason="Test ban", banned_for="testing")

        # Debug: check if ban was created
        ban_info = cache.get(f"ban_fp:{self.test_fingerprint}")
        print(f"DEBUG: ban_info = {ban_info}")
    ...
    def test_expired_ban_automatic_cleanup(self):
        ...
        with patch("users.services.BanService.unban_by_fp") as mock_unban:
            ...
            # Should not be redirected (ban expired - is_fp_banned returns False after unban)
            self.assertEqual(response.status_code, 200)
...
class MiddlewareIntegrationTest(TestCase):
    ...
    def test_middleware_order_independence(self):
        ...
        # Stack middleware
        middleware_stack = DeviceFingerprintMiddleware(dummy_middleware(final_response))
