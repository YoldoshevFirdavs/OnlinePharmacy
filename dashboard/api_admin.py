...
class AdminAnalyticsAPIView(APIView):
    ...
    def get(self, request):
        ...
        try:
            ...
            revenue_7d = Order.objects.filter(created_at__gte=last_7_days).aggregate(Sum('total_price'))['total_price__sum'] or 0
            revenue_30d = Order.objects.filter(created_at__gte=last_30_days).aggregate(Sum('total_price'))['total_price__sum'] or 0
            ...
            for i in range(30):
                ...
                day_start = day.replace(hour=0, minute=0, second=0, microsecond=0)
                day_end = day_start + timedelta(days=1)
                revenue = Order.objects.filter(created_at__gte=day_start, created_at__lt=day_end).aggregate(Sum('total_price'))['total_price__sum'] or 0
